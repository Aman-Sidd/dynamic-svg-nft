...
// '</svg>';
        string memory svg = deployer.getSadSvg();
        string memory expectedURI = deployer.svgToImageURI(svg);
        ...
        string memory generated_svg = string(abi.encodePacked(baseURL,svgBase64Encoded));

        console2.log("ExpectedURI:", expectedURI);
        assertEq(generated_svg, expectedURI);
    }
}
